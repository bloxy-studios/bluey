...
|---|---|---|---|
| Rust core | `cargo test -p bluey-core` | `bun run test:rust` | state machine matrix, event names ⇄ TS list, model routing & fallbacks, token budgeting, snapshot trimming, adapters, built-in modes, shortcut parsing/conflicts, session rules, text utils, provider presets + `.env` import planning |
...
8. **Batch transcription** — (when `ai_transcribe_file` lands) a WAV file transcribes with `gemini-3.5-transcribe`.
