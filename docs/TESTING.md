...
| Rust app crate | `cargo clippy --target aarch64-apple-darwin --no-default-features --features dev-tools --all-targets -- -D warnings` (Linux, type-check only) · `cd src-tauri && cargo test --features dev-tools` (macOS) | `bun run check:rust --darwin` | every module compiles for macOS incl. its unit tests (audio routing, transcription dedupe/rotation constants, Gemini adapter backoff, env import, the WebView secret gate, the reset failure report, the Keychain token JSON shape, the pending provider profiles, the sidecar env boundary, account token redaction) |
