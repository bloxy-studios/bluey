...
| `instructions` | the CLI sends the model's `model_messages.instructions_template` (13–21 KB Codex prompt) from the `/models` catalog; openai-oauth and CLIProxyAPI send `""` and report working streams; one June 2026 article claims server-side validation | `core/src/client.rs:794-835`; `protocol/src/openai_models.rs:534-552` | Bluey sends its own prompt as `instructions` and falls back to the template (Bluey's prompt as a `developer` item) on a 400 or an empty stream — the replay runs in production, sticky per process, logged once (*What Bluey sends* below). `""` was never shipped |
...
name, no `OpenAI-Beta`; body per the *Body* row with `instructions` = **Bluey's own system
prompt** (`InstructionsPolicy::Own` — the model is primed by Bluey's response contract and mode,
not by the 13–21 KB Codex agent template, and ~4–5 K prefill tokens are saved per request). The
`instructions` replay of the row above is probed in production instead of settled by hand: a 400
on that shape is retried once with the catalog's `instructions_template` as `instructions` and
Bluey's prompt as a `role: "developer"` input item, and a stream that finishes without a single
text delta switches the *next* request to the same template; either switch is sticky for the
process and logged once (`WARN … did not accept Bluey's prompt as instructions`; the quiet empty
reply itself surfaces in the HUD as *Couldn't read the answer*). `reasoning.effort` clamped to
