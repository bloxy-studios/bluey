...
export function OnboardingFlow() {
  ...
  return (
    <div data-tauri-drag-region className="flex h-screen flex-col bg-bg text-fg">
      <div data-tauri-drag-region className="flex shrink-0 items-center justify-center pt-6" aria-label={`Step ${index + 1} of ${STEPS.length}`}>
        <div className="flex items-center gap-1.5">
          {STEPS.map((s, i) => (
            <span
              key={s.id}
              className={cn("size-1.5 rounded-full transition-colors", i === index ? "bg-fg" : i < index ? "bg-fg-muted" : "bg-bg-tile")}
              aria-hidden
            />
          ))}
        </div>
      ...
      <div key={stepKey} className="flex min-h-0 flex-1 flex-col items-center justify-center px-12 motion-safe:animate-rise-in">
