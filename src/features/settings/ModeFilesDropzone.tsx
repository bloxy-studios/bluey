...
/** Dashed dropzone with the stacked-documents illustration (Modes → Files). */
export function ModeFilesDropzone({ modeId, onAdded }: ModeFilesDropzoneProps) {
  ...
  const browse = async () => {
    ...
    try {
      ...
      for (const path of paths) {
        await bluey.documents.add({ input: { kind: "notes", scope: "mode", scopeId: modeId, path } });
      }
      if (paths.length > 0) {
        showToast(paths.length === 1 ? "File added" : `${paths.length} files added`);
        onAdded();
      }
    } catch (error) {
      console.warn("[modes] pickFiles failed", error);
    } finally {
      setBusy(false);
    }
  };
    ...
    for (const file of files) {
      ...
      try {
        ...
        await bluey.documents.add({
          input: { title: file.name, kind: "notes", scope: "mode", scopeId: modeId, content, format: lower.endsWith(".md") ? "md" : "txt" },
        });
        added += 1;
      } catch (error) {
        console.warn("[modes] drop add failed", error);
      }
    }
    if (added > 0) {
      ...
      showToast("Use browse for PDF and DOCX files");
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="text-[15px] font-medium text-fg">Adding files gives more context to Bluey</div>
      <div className="text-[13px] text-fg-muted">
        Drag &amp; drop files here to add them, or <span className="font-medium text-accent">browse files</span>
      </div>
    </div>
  );
}
