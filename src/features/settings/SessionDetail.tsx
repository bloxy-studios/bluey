import { ArrowLeft, ChevronRight, Download, Plus, Trash2 } from "lucide-react";
...
import { toBlueyError, type BlueyError, type SessionDetail as SessionDetailData, type SessionEvent } from "@/lib/types";
...
function TimelineEvent({ event }: { event: SessionEvent }) {
  ...
  return (
    <li className="relative pl-6">
      ...
      >
        <span className="w-[52px] shrink-0 font-mono text-[11px] text-fg-subtle">{formatTime(event.createdAt)}</span>
          ...
          {event.refs ? (
            <div className="mt-1 flex flex-wrap gap-1.5">
              {Object.entries(event.refs).map(([key, value]) => (
                <span key={key} className="rounded bg-bg-tile px-1.5 py-0.5 font-mono text-[10.5px] text-fg-subtle">
                  {key}: {value}
                </span>
              ))}
            </div>
          ) : null}
          {event.confidence !== undefined ? <div className="mt-1 text-[11px] text-fg-subtle">confidence {(event.confidence * 100).toFixed(0)}%</div> : null}
        </div>
      ) : null}
    </li>
  );
...
export function SessionDetail({ sessionId, onBack }: { sessionId: string; onBack: () => void }) {
  ...
  const exportMarkdown = async () => {
    try {
      ...
      console.warn("[sessions] export failed", error);
    }
  };
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      const generated = await getEngine().summarizeSession({ session, mode, transcript, responses, events, notes, settings });
    ...
    <div className="mx-auto flex min-h-0 w-full max-w-[800px] flex-1 flex-col px-6">
      <div className="flex shrink-0 items-center gap-3 pb-3 pt-1">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-[16px] font-semibold text-fg">{session.title ?? "Untitled session"}</h2>
          <div className="text-[12.5px] text-fg-muted">
            {mode?.name ?? session.modeId} · {formatDateTime(session.startedAt)} · {formatDuration(session.startedAt, session.endedAt)}
          </div>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pb-12">
        <section className="mt-2">
          <h3 className="mb-1.5 text-[13px] font-semibold uppercase tracking-wide text-fg-subtle">Timeline</h3>
        ...
        {responses.length > 0 ? (
          <section className="mt-6">
            <h3 className="mb-2 text-[13px] font-semibold uppercase tracking-wide text-fg-subtle">Responses</h3>
            <div className="flex flex-col gap-3">
              {responses.map((response) => (
                <div key={response.id} className="rounded-card border border-border bg-bg-elevated p-4">
                  {response.prompt ? <div className="mb-2 text-[12.5px] text-fg-subtle">“{response.prompt}”</div> : null}
        ...
        <section className="mt-6">
          ...
          <div className="flex flex-col gap-1.5">
            {notes.map((n) => (
              <div key={n.id} className="rounded-[10px] border border-border bg-bg-elevated px-3.5 py-2.5 text-[13.5px] text-fg">
                {n.content}
              </div>
            ))}
        ...
        <section className="mt-6">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Button variant="secondary" size="sm" onClick={() => void generateSummary()} disabled={summarizing}>
              {summarizing ? <Spinner size={12} /> : null} {summary ? "Regenerate summary" : "Generate summary"}
            </Button>
          </div>
          {summaryError ? <ErrorBanner error={summaryError} onRetry={() => void generateSummary()} compact className="mb-2" /> : null}
          {summary ? (
              ...
              {(
                ...
                .map(([label, list]) => (
                  <div key={label} className="mt-3">
                    <div className="mb-1 text-[11.5px] font-medium uppercase tracking-wide text-fg-subtle">{label}</div>
